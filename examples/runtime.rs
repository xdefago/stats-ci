...
/// Measurements on a 2022 M2 MacBook Air (M2, 24GB RAM, 1TB SSD) with rustc 1.62.2 yield a runtime of
/// about 50ms for an array with 12.5M elements (when compiled in --release).
/// unfirtunately, the runtime seems to hang (or get very slow) when the array size is increased to
